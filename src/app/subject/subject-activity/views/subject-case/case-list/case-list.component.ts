...
export class CaseListComponent implements OnInit {
    ...
    loadData(page?: CuiPagination) {
        this.loading = true;
        this.offeringCaseService.searchData(this.subjectId, this.getParams(page)).subscribe(
            data => {
                this.data = data;
                this.loading = false;
            },
    ...
    private batchOperate(action: string, actionName: string, offeringCase?: OfferingCase, single_flag?: boolean) {
        ...
        if (canNext) {
            ...
            if (single_flag) {
                ...
                this.offeringCaseService[action](selected.map(it => it.id), this.subjectId).subscribe(
                    ok => {
    ...
    caseCiteLookupOk(selected) {
        if (selected && selected.length > 0) {
            this.offeringCaseService.citeCase(this.subjectId, selected).subscribe(
                ok => {
