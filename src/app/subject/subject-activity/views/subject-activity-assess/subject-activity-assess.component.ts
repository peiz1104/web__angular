...
@Component({
  selector: 'spk-subject-activity-assess',
  templateUrl: './subject-activity-assess.component.html',
  styleUrls: ['./subject-activity-assess.component.scss']
})
export class SubjectActivityAssessComponent implements OnInit {
  ...
  loadData(page?: CuiPagination) {
    ...
    let params = {
      ...
      assessType: 'SELF_SUBJECT',
      offeringId: this.subjectId,
      page: page ? page.number : 0,
      size: page ? page.size : '',
      sort: page && page.sort ? page.sort : ''
    };
    this.loading = true;
    this.paperService.trainList(this.subjectId, params).subscribe(
      pag => {
        this.pagination = pag;
  ...
  saveSourse() {
    ...
    if (this.sourseSelected) {
      ...
      this.paperService.copySourse(this.subjectId, params).subscribe(
        ok => {
          this.bLoading = false;
          this.isShowSourse = false;
          this.searchtext = '';
      ...
      this.bLoading = false;
      tipMessage('请至少选择一个评估', 'warning');
    }
  }

  toAnalysis(row) {
    // 判断是否有作答记录
    this.paperService.getTotalAnswer(this.subjectId, row.paper.id).subscribe(
      allAnswer => {
        if (allAnswer > 0) {
          this.router.navigate([row.paper.id, 'analysis'],
           { relativeTo: this.route, queryParams: {assessId: row.id, offeringId: this.subjectId, assessType: 'ASSESS'}});
        } else {
