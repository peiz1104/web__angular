 ...
 * @Last Modified time: 2018-04-17 15:11:18
 */
...
export class ExaminationService extends BaseService<any> {
  ...
  // 获取考试列表 （考试资源：培训班在用）
  getExamiList(parim?: any): Observable<Pagination<any>> {
  ...
  // 删除培训班考试
  deleteTrainExam(ids?: any[]) {
    return this.httpAdaptor.delete(
      "/api/exam/manager/deleteTrainingClassExam",
      { params: { ids: ids } }
    );
  }
  // 培训班发布取消发布
  publishTrainingClassExam(ids?: any[], putOut?: boolean): Observable<any> {
    return this.httpAdaptor.post(
      "/api/exam/manager/publishTrainingClassExam",
      null,
      { params: { ids: ids, isPutout: putOut } }
    );
  }
  ...
  setMarkTime(params): Observable<any> {
    return this.httpAdaptor.post(`/api/exam/staff/setMarkTime`, null, {
      params: params
    });
  }
  // 产品版阅卷借口
