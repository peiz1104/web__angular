...
export class ExamAddGroupComponent implements OnInit {
    ...
    showAddLoading: boolean = false; // model框确认后loading等待
    markStartTime;
    markEndTime;
    ...
    ngOnInit() {
        ...
        this.tabINDEX = this.route.snapshot.params["index"];
        switch (this.route.snapshot.params["sort"]) {
            case "PJ":
                this.groupTitle = "阅卷";
                break;
            case "FP":
        ...
        this.confirmServ.confirm({
            ...
            onOk() {
                that.examinationService
                    .deleteGroup({
                        markSettingId: that.markSettingId,
                        markGroupId: row.id
                    })
                    .subscribe(
                    data => {
    ...
    back() {
        ...
        //   index: this.tabINDEX
        // };
        // console.log("返回：", `/exam/examination/edit/${this.examId}/newAllot`);
        // this.router.navigate([
        //   `/exam/examination/edit/${this.examId}/newAllot`,
        //   obj
        // ]);
    }
    showUserAdd(row) {
