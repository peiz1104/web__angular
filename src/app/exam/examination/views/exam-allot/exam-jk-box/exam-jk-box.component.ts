...
export class ExamJkBoxComponent implements OnInit {
    ...
    clearJKcode() {
        if (!this.JKcolumnsData || this.JKcolumnsData.length == 0) {
            tipMessage('请先添加监考人！');
            return;
        }
        let that = this;
    ...
    ShowEditJKtimeModal() {
        ...
        this.ShowEditJKtime = true;
    }
    submitJKtime() {
        let ids = this.selection && this.selection.length > 0 && this.selection.map(it => it.id);
