...
export class ExamNewAllotComponent implements OnInit {
    ...
    FPcolumnsData: any;
    examEndTime: any;
    SPisLoading: boolean = false; // 审核确定loading
    ...
    showJK: boolean = false; // tab切换到JK
    showPJ: boolean = false; // tab切换到PJ
    showFP: boolean = false; // tab切换到FP
    showSP: boolean = false; // tab切换到SP
    showKA: boolean = false; // tab切换到KA
    userGroupId: any;
    userGroupName: any;
    constructor(
    ...
    ngOnInit() {
        // 异步加载下拉列表
        // 管理员类型
        this.selectedTabIndex = this.route.snapshot.params["index"] || 0;
        // this.selectedTabIndex=3
        console.log(this.selectedTabIndex, "selectedTabIndex")
        // this.showPJ=true;
        this.getMarkTypeData()
        ...
        this.route.data.subscribe((obj: { examination: any }) => {
            ...
            this.userGroupId = obj.examination['userGroupId'];
            this.userGroupName = obj.examination['userGroupName'];
            if (!this.examId) {
                    ...
                    req => {
                        if (!req.examId) {
                            this.confirmServ.warning({
                                ...
                                cancelText: '',
                                onOk: () => {
                                    this.router.navigate([`exam/examination/edit/${obj.examination['id']}/info`], {
                                        queryParams: { userGroupId: this.userGroupId, userGroupName: this.userGroupName }
                                    });
                                }
                            });
                            return;
    ...
    getMarkTypeData() {
        this.examinationService.getDictionaries({ dicType: 'STAFF_TYPE' }).subscribe(
            data => {
                ...
                console.log(this.markTypeData, "marktype")
            },
            err => {
                tipMessage(err);
            }
    ...
    showTab(code) {
        ...
        return this.markTypeData.indexOf(code) != -1
    ...
    showViewPaper(obj) {
        ...
        console.log('viewPaperParams:', this.viewPaperParams)
    }
    // 单人阅卷编辑提交
    submitPSEdit() {
        ...
        console.log(this.markStartTime, "markStartTime")
        console.log(this.markEndTime, "markEndTime")
        if (this.markEndTime < this.markStartTime) {
            tipMessage('开始时间不能晚于结束时间！', '', 4000);
            return;
        }
    ...
    submitAddGroup() {
        ...
        console.log(this.markEndTime, "this.markEndTime")
        console.log(this.markStartTime, "this.markStartTime")

        if (this.markEndTime < this.markStartTime) {
            tipMessage('开始时间不能晚于结束时间！', '', 3000);
            return;
        }
        let params = {
            markGroupName: this.markGroupName,
    ...
    SHhandleOk = data1 => {
        ...
        let sendData: any;
        let _SPTREEID = data1 && data1.map(item => item.id)
        sendData = {
            userGroupIds: _SPTREEID.toString(),
