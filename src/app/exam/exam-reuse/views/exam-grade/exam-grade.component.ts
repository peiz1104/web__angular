 ...
 * @Last Modified time: 2018-05-11 10:47:12
 */
...
export class ExamGradeComponent implements OnInit {
                    ...
                    this.examinationService.getOffering(obj.examination['id']).subscribe(
                        req => {
                            if (!req.examId) {
                                ...
                                console.log('this.cache.examId', this.cache.examId)
                                this.loadData();
                            }
                        },
                        err => tipMessage(err)
    ...
    toReview(row) {
        ...
        this.router.navigate([`/training/class/${this.cache.examWhId}/exam/${this.cache.examId}/review`], { queryParams: { 'userId': row.userId, 'examId': row.examId } })
    }
    // 时间验证方法
