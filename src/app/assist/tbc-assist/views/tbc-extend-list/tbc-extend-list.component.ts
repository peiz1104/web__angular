...
@Component({
    selector: 'spk-tbc-extend-list',
    templateUrl: './tbc-extend-list.component.html',
    styleUrls: ['./tbc-extend-list.component.scss']
})
export class TbcExtendListComponent implements OnInit {
    ...
    goExame(id, trainingLevelId, type) {
        this.tbcAssistApi.goexame(id, trainingLevelId).subscribe(
            res => {
                this.router.navigate([`/assist/tbc/extand/${id}/${type}`]);
            },
            err => {
                this.router.navigate([`/assist/tbc/extand/${id}/${type}`]);
            }
