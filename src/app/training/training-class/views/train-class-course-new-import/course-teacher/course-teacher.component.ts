...
export class CourseTeacherComponent implements OnInit {
    ...
    next() {
        ...
        if (this.current == 1 && this.loading) {
            return tipMessage('正在导入中...', 'warning');
        }
        this.current += 1;
    }
    ...
    getImportProgress() {
        this.offeringCourseApi.importProgressCourse(this.taskKey).subscribe(
            m => {
    ...
    // 检测文件是否正确
    checkExcel() {
        this.offeringCourseApi.importCheckExcel(this.taskKey).subscribe(
            res => {
                console.log(res);
                this.status = 'next';
                this.importErrInfo = undefined;
    ...
    import() {
        this.loading = true;
        this.offeringCourseApi.importCourse(this.taskKey).subscribe(
            res => {
                console.log(res)
                if (res.length !== 0) {
                    this.status = 'after';
