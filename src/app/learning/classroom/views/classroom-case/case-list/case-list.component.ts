...
export class CaseListComponent implements OnInit {
    ...
    columns: Column[] = [
        { title: '案例编号', data: 'caseCode', tpl: 'caseCode' },
    ...
    loadData(page?: CuiPagination) {
        this.loading = true;
        this.offeringCaseService.searchData(this.id, this.getParams(page)).subscribe(
            data => {
                this.data = data;
                this.loading = false;
            },
    ...
    /* publish_single(offeringCase?: OfferingCase) {
      this.batchOperate_single('publish', '发布', offeringCase);
      }
    disPublish_single(offeringCase?: OfferingCase) {
      this.batchOperate_single('disPublish', '撤消发布', offeringCase);
      }
    batchOperate_single(action: string, actionName: string, offeringCase?: OfferingCase) {
      if (this.publishCheck([offeringCase], actionName)) {
        this.offeringCaseService[action]([offeringCase].map(it => it.id), this.id).subscribe(
          ok => {
    ...
    private batchOperate(action: string, actionName: string, offeringCase?: OfferingCase, single_flag?: boolean) {
        ...
        if (!selected || selected.length == 0) {
            tipMessage(`请选择要${actionName}的案例`, '', 4000);
            return;
        }
        if (actionName != "删除") {
        ...
        if (canNext) {
            if (single_flag) {
                ...
                if (action == 'delete' && !canDel) {
                    tipMessage("发布状态下不允许删除", '', 4000);
                    return;
                }

                this.offeringCaseService[action](selected.map(it => it.id), this.id).subscribe(
                    ok => {
    ...
    publishCheck(selection: OfferingCase[], actionName: string) {
        ...
        for (let index = 0; index < selection.length; index++) {
            ...
            if (info.useType == "CITE") {
                tipMessage("所选案例中包含引用案例，不能进行该操作", '', 4000);
                canNext = false;
                break;
            }
            if (actionName == "发布") {
        ...
        if ((!infos || infos.length <= 0) && canNext) {
            canNext = false;
            switch (actionName) {
                case "发布": tipMessage("所选案例都处于发布状态，不能进行发布操作", '', 4000); break;
                case "撤销发布": tipMessage("所选案例都处于未发布状态，不能进行撤销发布操作", '', 4000); break;
            }
        }
        return canNext;
    }
    ...
    caseCiteLookupOk(selected) {
        if (selected && selected.length > 0) {
            this.offeringCaseService.citeCase(this.id, selected).subscribe(
                ok => {
